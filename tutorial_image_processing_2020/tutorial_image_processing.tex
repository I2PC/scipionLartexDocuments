...
\begin{flushright}
 \large
%\emph{Author:}\\
  \textsc{Carlos Oscar S. Sorzano \& Daniel Marchán} % Your name
\end{flushright}
%\end{minipage}
...
This tutorial requires, in addition to \scipion, \textit{cryoSPARC2} (\url{https://cryosparc.com/}). It is \textbf{IMPORTANT} to remark that for executing any \textit{cryoSPARC2} protocol, in the \scommand{compute settings} tab, the option \ttt{Cache particle images on SSD} has to be \textbf{deactivated}. 
...
\input{030-fromMoviesToMicrographs.tex}
\input{040-CTFEstimation.tex}
\input{050-ParticlePicking.tex}
\input{060-ExtractParticles.tex}
\input{070-2DClassification.tex}
\input{080-InitialVolume.tex}%%%
\input{090-3DClassificationANDRefinement.tex}
\input{010-VolumeEnhancementANDValidation.tex}
