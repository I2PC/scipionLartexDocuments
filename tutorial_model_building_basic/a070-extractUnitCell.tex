\section{Extract asymmetric unit protocol}
...
\begin{itemize}
  \item Requirements to run this protocol and visualize results:
    ...
    \end{itemize}
  \item \scipion menu:\\
  \ttt{Model building -> Preprocess map} (\ffigure{fig:app_protocol_extractUnitCell_1} (A))
  ...
  \begin{figure}[H]
    ...
    \caption{Protocol \scommand{xmipp3 - extract asymmetric unit}. A: Protocol location in \scipion menu. B: Protocol form.}
    \label{fig:app_protocol_extractUnitCell_1}
   \end{figure}
  
  \begin{itemize}
  \item \ttt{Input Volume}: Volume already downloaded in \scipion from which the asymmetric unit will be extracted.
  ...
  Types of rotational symmetry included in this protocol are shown in \ffigure{fig:app_protocol_extractUnitCell_2}. Two names appear in each case, the first one corresponds to \ttt{XMIPP} nomenclature of symmetry because we are using \ttt{XMIPP} package, and the second one (in brackets) follows the general \scipion nomenclature. Current \scipion nomenclature is $ChimeraX$'s nomenclature, which is, in turn, the same symmetry nomenclature of the International Union of Cristallography. 
    ...
    \caption{Protocol \scommand{xmipp3 - extract asymmetric unit}. Types of rotational symmetry.}
  ...
  \item Cyclic symmetry \ttt{Cn (Cn)}: Only one symmetry axis goes through the geometric center of the volume. Two more form parameters are shown when this type of symmetry is selected:
   \begin{itemize}
    \item \ttt{Symmetry Order}: Number of times (\ttt{n}) in which a volume shows the same shape when the volume rotates around the symmetry axis from 0 to 360º. If the same shape is only obtained after turning 360º, then \ttt{n = 1}. This means that the volume has no symmetry. \ttt{360º/n} determines the rotation angle. 
	\item \ttt{offset}: Starting angle around Z axis. 
   \end{itemize}
  \item Dihedral symmetry \ttt{Dn (Dxn)}: Two perpendicular symmetry axes go through the geometric center of the volume. As in the case of cyclic symmetry, two more form parameters are shown when this type of symmetry is selected:
   \begin{itemize}
	...
	\item \ttt{offset}: Starting angle around Z axis. 
   \end{itemize}
  \item Tetrahedral symmetry \ttt{T (T222)}: Four symmetry axes go from each vertex to the opposing face center (order 3), and three symmetry axes join opposing edges (order 2). \ttt{Symmetry order = 12}.
  ...
  \item Icosahedral symmetries \ttt{I1 (I222), I2 (I222r), I3 (In25), I4 (In25r)}: Six symmetry axes join opposing vertices (order 5), 10 symmetry axes join baricenters of opposing faces (order 3), and 15 symmetry axes join opposing edges (order 2). \ttt{Symmetry order = 60}. Each type of icosahedral symmetry depends on its initial orientation. Check in \chimera (\url{https://www.cgl.ucsf.edu/chimerax/docs/user/commands/shape.html}) each icosahedral symmetry by writing in the command line: \ttt{shape icosahedron radius 50 orientation} (\ttt{222}: default, order 2 axes follow XYZ coordinate axes; \ttt{222r}: idem rotated 90º around Z axis; \ttt{n25}: an order 2 axis and an order 5 axis follow Y and Z axes, respectively, \ttt{n25r}: idem rotated 90º around Z axis.
  \end{itemize}
  
  \item \ttt{Inner Radius (px)}: Minimal distance from the geometric center that delimits inwards the part of the electron density map that will be included in the extracted volume. A wizard symbol on the right side of this parameter can be helpful to select this radius.
  \item \ttt{Outer Radius (px)}: Maximal distance from the geometric center that delimits outwards the part of the electron density map to be included in the extracted volume. In other words, the part extracted of the map electron density will be between the \ttt{Inner} and the \ttt{Outer Radius}. Again, the wizard symbol on the right side of this parameter can be helpful to select this radius.  
  ...
  After executing the protocol, press \ttt{Analyze Results} and a small window will be opened (\ffigure{fig:app_protocol_volume_2}). This window allows you to select between \ttt{chimerax} ($ChimeraX$ graphics window) and \ttt{slices} ($ShowJ$, the default \scipion viewer), to visualize the volume.
 ...
 \item Summary content:
  \begin{itemize}
     \item Protocol output (below \scipion framework):\\ \ttt{xmipp3 - extract asymmetric unit -> ouputVolume};\\ Volume (x, y, and z dimensions, sampling rate).
