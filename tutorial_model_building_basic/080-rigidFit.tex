...
Once we have the predicted \ttt{model} of any structural element included in our map, to fit that \ttt{model} in the volume constitutes the next step in the modeling workflow. Two protocols have been included in \scipion with this purpose, \scommand{phenix - dock in map} (Appendix \ref{app:dockInMapProtocol}, \citep{Liebschner2019}) and \scommand{chimerax - rigid fit} (Appendix \ref{app:chimeraRigidFit}). The first one allows automatic fitting of models in maps, while the second one only does it when model and map are quite close, thus requiring manual fitting in advance. Although there is no a general rule to fit \iii{map} and \iii{model}, because it will depend on the particular problem and on our previous knowledge, in this tutorial we are going to use \phenix \ttt{dock in map} application first, followed by the final \ttt{Fit in Map} in \chimera \ttt{rigid fit}. Observe these two new steps in the modeling \scipion workflow in \ffigure{fig:scipion_workflow_rigidfit}.
...
\subsection*{Initial rigid fit with \phenix \ttt{dock in map}}
 ...
 Open the protocol \scommand{chimerax - rigid fit}, include again the map of the asymmetrical unit (2), and the just fitted \iii{model} of the human \ttt{metHgb} $\alpha$ subunit (3), and execute the protocol (4).
 ...
 \begin{figure}[H]
  ...
  To track the \chimera fitted model in \scipion we have to save it as fitted $model$ of the \ttt{metHgb} $\alpha$ subunit in the \chimera command line before closing the \chimera window:\\
