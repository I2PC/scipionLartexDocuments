...
that you can see in the \ttt{Summary} of the \scipion framework after changing the \ttt{Advanced} parameter \ttt{Local grid search} (5) from \ttt{Yes} to \ttt{No} (default value). 
 ...
 \begin{figure}[H]
  ...
  After executing the \scommand{phenix - search fit} protocol (\ffigure{fig:phenix_search_fit_3}(8)) we can have a look to the results. By pressing \ttt{Analyze Results} (\ffigure{fig:phenix_search_fit_3}(9)) a window with the Viewer menu is opened (\ffigure{fig:phenix_search_fit_4}(A)). This menu allows to visualize a certain number of  atomic structures, according to their ranking scores, with lateral side chains fitted in the map density (1). We have selected the best 5 structures in this case, which will be opened in \chimera (2) surrounded by the density located at 3.0\AA\ of the structure (3). The summary plot of scores from map model fitting can be visualized in (4).
  
   \begin{figure}[H]
  \centering 
  \captionsetup{width=.9\linewidth} 
  \includegraphics[width=0.85\textwidth]{Images/Fig_search_fit_4.pdf}
  \caption{Visualization of \scommand{phenix - search fit} protocol results in \scipion. A. Results menu. B. Model panel in \chimera showing the selected atomic structures. C. Map-model fit score plot. D. Models 3 to 8 displayed inside the map density. E. Models 3 and 4. F. Model 4 compared with the same fragment of the atomic structure \ttt{5NI1}.}
  \label{fig:phenix_search_fit_4}
  \end{figure}
  
  Panel B of the \ffigure{fig:phenix_search_fit_4} shows the \chimerax model panel with the five best selected structures, 4, 9, 11, 20 and 23. The purple arrow points the position of these indexes. The respective score values can be observed in \ffigure{fig:phenix_search_fit_4} (C) where five purple arrows point to those five score peaks. \ffigure{fig:phenix_search_fit_4} (D) details the view of the five structures in the GUI of \chimera (models 4 to 8), as well as the input fragment of ALA (model 3). Remark that lateral side chains are not shown by default. \ffigure{fig:phenix_search_fit_4} (E) compares this input model 3 with the structure of best score (model 4). To display the lateral side chains select each model and press ``Show'' in ``Atoms'' section of the toolbar. In the same way, \ffigure{fig:phenix_search_fit_4} (F) compares this input model 3 with the respective fragment of the \ttt{PDB ID 5NI1} structure. To open it as model \ttt{\#9} and align it with the rest of the structures, write in the \chimera comand line:\\
  \ttt{open 5ni1}\\
