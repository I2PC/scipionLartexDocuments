...
that you can see in the \ttt{Summary} of the \scipion framework after changing the \ttt{Advanced} parameter \ttt{Local grid search} (5) from \ttt{Yes} to \ttt{No} (default value). 
 ...
 \begin{figure}[H]
  ...
  This example shows a small fraction of residues from the \ttt{metHgb} $\alpha$ subunit that was not completely modeled, except for the skeleton of $\alpha$ carbons. The sequence of the chain is perfectly known, but for certain residues we were unable of tracing the lateral side chains of those residues and only \ttt{ALA} residues appear in our atomic structure. A detail of the small fragment of \ttt{ALA} residues can be observed in the \ffigure{fig:phenix_search_fit_2} (red arrows). The protocol \phenix \ttt{search fit} might help us to replace the \ttt{ALA} residues by the appropriate aminoacids.
  ...
  As the \ffigure{fig:phenix_search_fit_1} indicates, the protocol \scommand{phenix - search fit} (4) requires three different inputs (1, 2 and 3):
  ...
  \item Small fragment of atomic structure that contains the \ttt{ALA} small chain. To create this fragment we start from the published atomic structure of the human \ttt{metHgb} $\alpha$ subunit (included in the model of the \ttt{PDB} ID \ttt{5NI1}, which can be downloaded from the database using the protocol \scommand{import atomic structure}. Next, we use the protocol \chimera \ttt{operate} to isolate the chain \ttt{A} of the structure. The atomic structure \ttt{5NI1} is the only one input of the protocol \chimera \ttt{operate}. After the opening of \chimera, write in the command line:\\
  \ttt{sel #2 \& \~\ \#2/A}\\
  \ttt{del sel}\\
  ...
  \ttt{del sel}\\
  \ttt{mmaker \#9 to \#4}\\
  In spite of some small differences, most of lateral side chains align quite well with the ones of the model \ttt{5NI1} traced on the experimental map. Then, the best approximation of the atomic structure retrieved (model \ttt{\#4}) can be selected to help with the tracing of a small fraction of the density map since the model \ttt{\#4} seems to be quite close to the actual atomic structure used as control (\ttt{PDB ID 5NI1}).
